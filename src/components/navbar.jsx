...
import { Menu, X, LayoutDashboard, LogIn, ArrowRight, Download } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden lg:flex space-x-6 items-center">
            ...
            <div className={`border-l pl-6 h-6 flex items-center gap-4 ${isNavbarLight ? "border-muted" : "border-slate-800"}`}>
              <a
                href="/dps-student-portal.apk"
                download
                className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-md text-xs font-bold transition-all border ${
                  isNavbarLight
                    ? "bg-amber-50 hover:bg-amber-100 border-amber-250 text-amber-800"
                    : "bg-amber-950/20 hover:bg-amber-950/40 border-amber-800/50 text-amber-300"
                }`}
              >
                <img
                  src="/logo.png"
                  alt="DPS Logo"
                  className="w-5 h-5 object-contain rounded-full shrink-0 bg-white p-0.5 border border-slate-200/50"
                />
              ...
              {user ? (
                <Link
                  href={user?.role ? `/portal/${user.role.toLowerCase()}` : "/sign-in"}
      ...
      {isOpen && (
        <div className="lg:hidden animate-fade-in bg-background border-b shadow-lg absolute w-full left-0">
          <div className="px-2 pt-2 pb-6 space-y-1 sm:px-3">
            ...
            <div className="pt-4 pb-2 border-t border-muted px-3 flex flex-col gap-3">
              <a
                href="/dps-student-portal.apk"
                download
                className="flex w-full items-center justify-center gap-2.5 bg-amber-50 border border-amber-200 text-amber-800 py-3 rounded-md text-base font-semibold transition-all shadow-sm"
              >
                <img
                  src="/logo.png"
                  alt="DPS Logo"
                  className="w-6 h-6 object-contain rounded-full shrink-0 bg-white p-0.5 border border-slate-200/50"
                />
                Android App for Students
              </a>
