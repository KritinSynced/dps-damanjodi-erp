...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuConfig: { [key: string]: { label: string; tab: string; icon: any }[] } = {
    STUDENT: [
      ...
      { label: "Fees & Payment", tab: "fees", icon: <CreditCard size={18} /> },
      { label: "Leave Request", tab: "leave", icon: <User size={18} /> },
    ],
    PARENT: [
      { label: "Dashboard", tab: "dashboard", icon: <LayoutDashboard size={18} /> },
      { label: "Child Progress", tab: "progress", icon: <FileSpreadsheet size={18} /> },
      { label: "Fee Payments", tab: "fees", icon: <CreditCard size={18} /> },
      { label: "Bus Route GPS", tab: "transport", icon: <Bus size={18} /> },
      { label: "Communication", tab: "chat", icon: <MessageSquare size={18} /> },
      { label: "Leave Requests", tab: "leave", icon: <User size={18} /> },
    ],
    TEACHER: [
      { label: "Dashboard", tab: "dashboard", icon: <LayoutDashboard size={18} /> },
      ...
      { label: "Assignments Uploader", tab: "assignments", icon: <BookOpen size={18} /> },
    ...
    ADMIN: [
      ...
      { label: "Transport Routes", tab: "transport", icon: <Bus size={18} /> },
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col md:flex-row select-none">
      ...
      {/* 1. DESKTOP SIDEBAR */}
      <DesktopSidebar menu={currentMenu} user={user} logout={logout} />

      {/* 2. MOBILE HEADER & NAVIGATION */}
      <MobileHeader menu={currentMenu} user={user} logout={logout} />
      ...
      {/* 4. MOBILE BOTTOM NAVIGATION (For Student/Parent - Touch friendly) */}
...
function DesktopSidebar({ menu, user, logout }: { menu: any[]; user: any; logout: () => void }) {
  return (
    <React.Suspense fallback={<DesktopSidebarInner menu={menu} user={user} logout={logout} currentTab="" />}>
      <DesktopSidebarWithParams menu={menu} user={user} logout={logout} />
    </React.Suspense>
  );
}

function DesktopSidebarWithParams({ menu, user, logout }: { menu: any[]; user: any; logout: () => void }) {
  const searchParams = useSearchParams();
  const currentTab = searchParams.get("tab") || "dashboard";
  return <DesktopSidebarInner menu={menu} user={user} logout={logout} currentTab={currentTab} />;
}

function DesktopSidebarInner({ menu, user, logout, currentTab }: { menu: any[]; user: any; logout: () => void; currentTab: string }) {
  return (
...
function MobileHeader({ menu, user, logout }: { menu: any[]; user: any; logout: () => void }) {
  return (
    <React.Suspense fallback={<MobileHeaderInner menu={menu} user={user} logout={logout} currentTab="" />}>
      <MobileHeaderWithParams menu={menu} user={user} logout={logout} />
    </React.Suspense>
  );
}

function MobileHeaderWithParams({ menu, user, logout }: { menu: any[]; user: any; logout: () => void }) {
  const searchParams = useSearchParams();
  const currentTab = searchParams.get("tab") || "dashboard";
  return <MobileHeaderInner menu={menu} user={user} logout={logout} currentTab={currentTab} />;
}

function MobileHeaderInner({ menu, user, logout, currentTab }: { menu: any[]; user: any; logout: () => void; currentTab: string }) {
...
function MobileBottomNav({ menu, user }: { menu: any[]; user: any }) {
  return (
...
function MobileBottomNavWithParams({ menu, user }: { menu: any[]; user: any }) {
  const searchParams = useSearchParams();
  const currentTab = searchParams.get("tab") || "dashboard";
  return <MobileBottomNavInner menu={menu} user={user} currentTab={currentTab} />;
}

function MobileBottomNavInner({ menu, user, currentTab }: { menu: any[]; user: any; currentTab: string }) {
  return (
