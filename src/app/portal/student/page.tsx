...
function StudentDashboardContent() {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/portal/student?studentId=${user?.studentId}`);
  ...
  const handlePayFeeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Update local fee state
        if (studentData) {
          const updatedFees = studentData.feePayments.map((f: any) => 
            f.id === paymentModal.id ? data.payment : f
          );
          setStudentData({ ...studentData, feePayments: updatedFees });
        }
  ...
  const presentDays = studentData.attendance.filter((a: any) => a.status === "PRESENT" || a.status === "LATE").length;
            ...
            <div className="bg-card border p-4 rounded-lg shadow-sm">
              ...
              <div className="flex items-baseline gap-1.5 mt-1.5">
                <span className="text-xl sm:text-2xl font-extrabold text-red-600">
                  ₹ {studentData.feePayments.filter((f: any) => f.status === "UNPAID").reduce((sum: number, f: any) => sum + f.amount, 0)}
                </span>
                <span className="text-[10px] text-muted-foreground">(Q4 Pending)</span>
              </div>
            </div>
            ...
            <div className="bg-card border p-4 rounded-lg shadow-sm">
              ...
              <div className="flex items-baseline gap-1.5 mt-1.5">
                <span className="text-xl sm:text-2xl font-extrabold text-slate-800 dark:text-slate-200">
                  {studentData.bookIssues.filter((b: any) => b.status === "ISSUED").length}
                </span>
                ...
                <div className="bg-card border border-primary/30 p-4 rounded-lg shadow-md space-y-4 relative overflow-hidden print-card">
                  ...
                  <div className="flex gap-4 items-center py-2">
                    <div className="w-16 h-16 rounded-full overflow-hidden bg-slate-100 shrink-0 border border-slate-200">
                      <img src={studentData.user.avatar} alt="Student avatar" className="w-full h-full object-cover" />
                    </div>
          ...
          <div className="grid grid-cols-3 gap-4 border-b pb-6">
            ...
            <div className="text-center space-y-1">
              <span className="block text-xl sm:text-2xl font-extrabold text-red-600">
                {studentData.attendance.filter((a: any) => a.status === "ABSENT").length}
              </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-900/30 border p-4 rounded-lg text-center text-sm">
              <div className="space-y-1">
                ...
                <span className="block text-base font-extrabold text-slate-800 dark:text-white">
                  {studentData.grades.filter((g: any) => g.examType === "MID_TERM").reduce((sum: number, g: any) => sum + g.theoryMarks, 0)}
                  /
                  {studentData.grades.filter((g: any) => g.examType === "MID_TERM").reduce((sum: number, g: any) => sum + g.maxMarks, 0)}
                </span>
              </div>
              <div className="space-y-1 border-x px-2">
                ...
                <span className="block text-base font-extrabold text-primary">
                  {(
                    (studentData.grades.filter((g: any) => g.examType === "MID_TERM").reduce((sum: number, g: any) => sum + g.theoryMarks, 0) /
                     studentData.grades.filter((g: any) => g.examType === "MID_TERM").reduce((sum: number, g: any) => sum + g.maxMarks, 0)) * 100
                  ).toFixed(1)}%
                </span>
              </div>
        ...
        <div className="bg-card border rounded-xl p-5 sm:p-6 shadow-sm space-y-6 animate-fade-in">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/30 border p-4 rounded-lg flex flex-wrap justify-between items-center gap-4 text-sm">
            ...
            <div className="text-xs text-muted-foreground">
              Note: Net banking, UPI, and Credit Card payments are supported online.
            </div>
          </div>

          {/* Dues list */}
          <div className="space-y-4">
            <h3 className="font-bold text-sm">Fee Bills Registry</h3>
            ...
            <div className="overflow-x-auto border rounded-lg bg-card text-xs sm:text-sm">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y text-muted-foreground">
                  {studentData.feePayments.map((fee: any) => (
                    <tr key={fee.id}>
            ...
            <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
              <div className="bg-card border max-w-md w-full rounded-xl overflow-hidden shadow-2xl p-6 space-y-6">
                <div className="border-b pb-3 flex justify-between items-center">
                  <h3 className="font-bold text-base text-slate-900 dark:text-white">ERP Fee Checkout</h3>
