...
def create_student(payload: dict, db = Depends(get_db)):
    ...
    try:
        ...
        if not parent:
            ...
            parent_password_hash = bcrypt.hashpw("parent123".encode("utf-8"), bcrypt.gensalt(10)).decode("utf-8")
            insert_p_user = text('''
        ...
        student_password_hash = bcrypt.hashpw("student123".encode("utf-8"), bcrypt.gensalt(10)).decode("utf-8")
        insert_s_user = text('''
            INSERT INTO "User" (id, email, "passwordHash", role, name, avatar, "createdAt", "updatedAt")
...
def create_teacher(payload: dict, db = Depends(get_db)):
    ...
    try:
        ...
        teacher_password_hash = bcrypt.hashpw("teacher123".encode("utf-8"), bcrypt.gensalt(10)).decode("utf-8")
        insert_user = text('''
            INSERT INTO "User" (id, email, "passwordHash", role, name, avatar, "createdAt", "updatedAt")
